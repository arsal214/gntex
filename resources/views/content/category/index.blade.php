    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="card">
            <div class="border-bottom card-header d-flex justify-content-between align-items-center">
                ...
                <div>
                    <a href="{{ route('categories.create') }}" class="btn btn-primary" data-placement="left">
                        {{ __('Create New') }}
                    </a>
                </div>
            </div>
            ...
            <div class="card-datatable table-responsive">
                <table class="datatables-users table" id="datatable">
                    ...
                    <tbody>
                        @foreach ($categories as $i => $list)
                            <tr>
                                <td></td>
                                <td>{{ ++$i }}</td>
                                <td>{{ $list->category }}</td>
                                <td>
                                    <a href="{{ route('categories.edit', $list->id) }}"
                                       class="btn rounded-pill btn-icon btn-label-primary waves-effect ml-2"><i
                                            class="fa fa-fw fa-edit"></i> </a>
                                        <form action="{{ route('categories.destroy', $list->id) }}" class="d-inline"
                                            method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
